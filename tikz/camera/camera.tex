...
\begin{tikzpicture}
  ...
  %Vorzeichen
  \fill[fill=blue] (-5.8, -6.15) circle [radius=0.3];
  ...
  \node[textStyle] at (4.8,5) {\Huge \textbf{Theta2}};
  \node[textStyle] at (-3,3) {\Huge \textbf{Con Core}};
  \node[textStyle] at (-5,1) {\Huge \textbf{Size}};
  \node[textStyle] at (-6,-9) {\Huge \textbf{Vorzeichen B}};
  \node[textStyle] at (6,7.5) {\Huge \textbf{Vorzeichen A}};
  \node[textStyle] at (-2,6) {\Huge \textbf{Quellposition}};

  %Arrows
  \draw[pointer] (-6,-9) .. controls (-6.7,-7.5) .. (-5.8,-6.15);
  \draw[pointer] (6,7.5) .. controls (7.5,6) .. (6.6,4);
