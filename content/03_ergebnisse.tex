...
\textbf{Ziel der Gamma/Hadron-Separation ist es, dass zwei-Klassen-Problem, bestehend aus Signal und Untergrund, zu lösen. 
Dazu wird versucht die Signalrate zu rekonstruieren um die Flüsse so wie die Spektren von Gamma Quellen zu bestimmen.}
...
\textbf{Da die simulierten nicht immer den Verteilungen des gemessen Protonen folgen, bestehen Unstimmigkeiten auf den Monte Carlo-Daten (Mismatch). 
Durch die Verwendung des gemessen Untergrund anstelle der simulierten Protonen soll der Mismatch minimiert werden und die Klassifikation verbessert.}
...
\begin{figure}[H]
  ...
  \label{fig:thetacut}
\end{figure}
Durch Schnitte auf der $\theta^{2}$ Verteilung soll ein möglichst reines Testset erstellt werden. 
...
Für Monte Carlo basierte Modelle nimmt die Signifikanz der Quelle nicht ab, wenn Protonen mit $\theta^{2} < 1$ aus dem Trainingsdatensatz entfernt werden. 
Desweiteren ist das Verhältniss an Gamma-Ereignissen $\theta^{2} > 0.5$ im Untergrunddatensatz gering, sodass diese als diffuse Gamma, oder Gamma Ereignissen welche hadronischen ähnlich sehen angesehen werden können. 
...
\end{enumerate}
Dabei wird auf dem Monte Carlo-Protonen Datensatz kein $\theta^{2}$-Schnitt durchgeführt, weil dieser frei von Gamma-Ereignissen ist.
...
Mithilfe eines Parameternetz wird die optimale Konstellation der Parameter gesucht. 
\begin{figure}
...
Desweiteren ist der maximale AUC-Wert, bei dem auf Messdaten trainierten Lerner, größer als bei dem auf Monte Carlo-Daten trainierte.
...
\begin{figure}[H]
  ...
  \label{fig:confdist}
\end{figure}
Dies liegt dem physikalischem Problem zugrunde, dass ein Proton, welches beim Eintritt in die Atmosphäre schon früh in ein $\pi^{0}$ Meson und einem $e^{+}$ zerfällt und kaum von einem elektromagnetischen Schauer zu unterscheiden ist wie in Kapitel \ref{sec:cherenkov} genauer beschrieben ist.
